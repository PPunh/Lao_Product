...
# load_dotenv before load BASE_DIR
...
LOGOUT_URL = reverse_lazy('sales:sale_page')
...
AXES_IP_LOCKOUT_MINUTES = 60 # Block duration for IP
...
# Configure the settings so that the website still knows it's HTTPS even though it's running plain HTTP.
